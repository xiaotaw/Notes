...
template <> inline int GetCvType<ushort>() { return CV_16UC1; }

template <> inline int GetCvType<uchar4>() { return CV_8UC4; }
...
template <typename T, unsigned layers> class Pyramid {
public:
  ...
  // ctor
  Pyramid(const unsigned rows, const unsigned cols,
          InterpolationMode interpolation_mode, const float scale_factor = 0.5)
      : base_rows_(rows), base_cols_(cols), scale_factor_(scale_factor),
  ...
  inline cv::Mat DownloadHelix(cudaStream_t stream) {
    ...
    if (layers == 0) {
      ...
      img = cv::Mat::zeros(base_rows_, base_cols_ + pyd_[1]->cols(),
                           GetCvType<T>());
    }
    // concat images
