...
template <typename T> __device__ float Gauss1D(T x, const float &sigma) {
  return exp(-x * x / (2.0f * sigma * sigma));
}

template <typename T> __device__ float Gauss2D(T x, T y, const float &sigma) {
...
void BilateralFilter(const DeviceArray2D<ushort> depth_in,
                     ...
                     const float &sigma_r, cudaStream_t) {
  dim3 blk(16, 16);
  dim3 grid(DivideUp(depth_in.cols(), blk.x), DivideUp(depth_in.rows(), blk.y));
