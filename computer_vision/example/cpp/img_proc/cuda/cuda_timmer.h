#pragma once
// code comes from https://github.com/niessner/BundleFusion/blob/master/FriedLiver/Source/SiftGPU/CUDATimer.h
// it's just a simple tool, todo: refactor timmer
...
public:
  ...
  void evaluate(bool showSum = false, bool showMax = false) {
    ...
    for (int i = 0; i < int(timingEvents.size()); ++i) {
    ...
    printf("--------------------------+----------+-----------+----------\n");
    for (int i = 0; i < int(aggregateTimingNames.size()); ++i) {
      printf("--------------------------+----------+-----------+----------\n");
