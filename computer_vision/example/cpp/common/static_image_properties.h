...
GetDepthModeRange(const k4a_depth_mode_t depthMode) {
  switch (depthMode) {
...
GetDepthDimensions(const k4a_depth_mode_t depthMode) {
  switch (depthMode) {
  case K4A_DEPTH_MODE_NFOV_2X2BINNED:
    return {320, 288};
  case K4A_DEPTH_MODE_NFOV_UNBINNED:
...
inline std::pair<uint16_t, uint16_t>
GetIrLevels(const k4a_depth_mode_t depthMode) {
  switch (depthMode) {
  case K4A_DEPTH_MODE_PASSIVE_IR:
...
using DepthPixelVisualizationFunction = Pixel(const DepthPixel &value,
...
// Functions that provide ways to take depth images and turn them into color
// representations suitable for showing to humans.
//
class DepthPixelColorizer {
public:
  // Computes a color representation of a depth pixel on the blue-red spectrum,
  // using min as the value for blue and max as the value for red.
  //
  static inline Pixel ColorizeBlueToRed(const DepthPixel &depthPixel,
    ...
    // If the pixel is actual zero and not just below the min value, make it
    // black
    //
    if (depthPixel == 0) {
      return result;
  ...
  //
  static inline Pixel ColorizeGreyscale(const DepthPixel &value,
                                        const DepthPixel &min,
                                        const DepthPixel &max) {
    // Clamp to max
    //
    ...
    const auto normalizedValue = static_cast<uint8_t>(
        (pixelValue - min) * (double(PixelMax) / (max - min)));
