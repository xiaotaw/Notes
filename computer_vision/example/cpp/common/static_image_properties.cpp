...
void ColorizeDepthImage(const k4a::image &depthImage,
  ...
  if (imageFormat != K4A_IMAGE_FORMAT_DEPTH16 &&
      imageFormat != K4A_IMAGE_FORMAT_IR16) {
  ...
  const uint16_t *depthData =
      reinterpret_cast<const uint16_t *>(depthImage.get_buffer());
  for (int h = 0; h < height; ++h) {
    for (int w = 0; w < width; ++w) {
      const size_t currentPixel = static_cast<size_t>(h * width + w);
      (*buffer)[currentPixel] =
          visualizationFn(depthData[currentPixel], expectedValueRange.first,
...
void ColorizeDepthImage(const cv::Mat depthImage,
  ...
  const uint16_t *depthdata =
      reinterpret_cast<const uint16_t *>(depthImage.data);
