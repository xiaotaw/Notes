...
public:
   ...
   * Output: scenes_
   */
  static int FindAllScenes(const std::string &root_path,
...
private:
  ...
  // extract timestamp from filename, e.g. d-1315332943.792050-3485245013.pgm ->
  // 1315332943.792050
  ...
  /**
   * Desc: sync depth and color images for a scene, just follow
   * nyu_v2_sync_color_depth_image.py However, in fact, we cannot sync depth and
   * color, as color and depth images are recoreded independently. NOTES: this
   * function NEED optimization. Input: depth_filenames_, color_filenames_
   * Output: synced_
   */
 ...
 *            and the color and depth files were extracted out by using
 * mat_to_png.py.
 */
