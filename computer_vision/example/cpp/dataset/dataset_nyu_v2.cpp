...
NyuV2RawDataset::NyuV2RawDataset(const std::string &data_path)
...
int NyuV2RawDataset::SyncDepthColorImage(bool verbose) {
  ...
  // the difference of timestamps when depth and color images were taken should
  // be less than TIME_ERROR_LIMIT
  ...
  if (verbose) {
    if (diff.size() > 0) {
      ...
      std::cout << "Sync depth and color for scene: " << data_path_
                << std::endl;
      std::cout << "  depth images: " << raw_depth_filenames_.size();
      std::cout << "; color images: " << raw_color_filenames_.size()
                << std::endl;
      std::cout << "  synced images: " << diff.size() << std::endl;
      std::cout << "  mean time diff: " << mean << std::endl;
    } else {
      std::cout << "[Error] no synced images for scene: " << data_path_
                << std::endl;
    }
  }
...
double NyuV2RawDataset::FilenameToTimestamp(const std::string &filename) {
  ...
  if (tokens.size() != 3) {
    std::cout << "[Error] cannot extract timestamp from " << filename
              << std::endl;
    return -1;
  } else {
...
// NyuV2RawDataset::NyuV2RawDataset(std::string data_path) :
// data_path_(data_path)
// {
//     FindAllScenes();
// }

int NyuV2RawDataset::FindAllScenes(const std::string &root_path,
...
NyuV2LabeledDataset::NyuV2LabeledDataset(std::string data_path)
    : RgbdDataset(data_path) {
  FindAllImages();
  ...
  color_camera_params_ =
      CameraParams(480, 640, 5.1885790117450188e+02, 5.1946961112127485e+02,
                   3.2558244941119034e+02, 2.5373616633400465e+02);
}
