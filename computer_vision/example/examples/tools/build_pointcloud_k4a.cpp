...
static void HelpInfo() {
  std::cout
      << "Record depth_image, color_image, and imu data of Azure Kinect Camera."
      << std::endl;
  std::cout
      << "The depth images are mapped to color camera by k4a's api, and both "
      << std::endl;
  std::cout
      << "depth and color images are resized with 480*640, and undistorted."
      << std::endl;
  ...
  std::cout << "    ./executable save_dir save_colorized_depth "
               "min_depth_for_colorization max_depth_for_colorization"
            << std::endl;
  std::cout << "Note1: " << std::endl;
  std::cout << "    save_dir -> string, save_colorized_depth -> {0, 1}, "
               "min/max_depth_for_colorization -> uint16_t"
            << std::endl;
  std::cout << "Note2: " << std::endl;
  std::cout << "    press 'q' or ctrl+C to exit, press 'p' to pause recording, "
               "press 'c'  or 'r' to continue recording"
            << std::endl;
}
...
int main(int argc, char *argv[]) {
  ...
  std::cout << "Done: found " << device_count << " K4A device(s). "
            << std::endl;
  // 打开（默认）设备
  ...
  std::vector<double> _camera_matrix = {intrinsics.fx / factor,
                                        0.f,
  ...
  cv::Size src_color_size = {
      calibration.color_camera_calibration.resolution_width,
      calibration.color_camera_calibration.resolution_height};
  cv::Size tgt_color_size = {int(src_color_size.width / factor),
                             int(src_color_size.height / factor)};
  ...
  std::cout << "Done: get transformation from depth to color, get undistort map"
            << std::endl;
  ...
  cv::Mat cv_rgba_image, cv_rgb_image, cv_scaled_rgb_image,
      cv_undistorted_rgb_image;
  cv::Mat cv_depth_image, cv_transformed_depth_image, cv_scaled_depth_image,
      cv_undistorted_depth_image, cv_colorized_depth_image;

  // 稳定设备，并计算device与system时间差
  std::vector<TimeVal> depth_image_time_offsets, color_image_time_offsets,
      imu_time_offsets;
  for (size_t i = 0; i <= 30;) {
    if (device.get_capture(&capture, std::chrono::milliseconds(1000))) {
      ...
      TimeVal depth_offset =
          (sys_time - depth_image.get_device_timestamp()).Validate();
      depth_image_time_offsets.push_back(depth_offset);
      // 计算color image的device time和system time之间的差
      color_image = capture.get_color_image();
      TimeVal color_offset =
          (sys_time - color_image.get_device_timestamp()).Validate();
      color_image_time_offsets.push_back(color_offset);
      // 获取IMU数据，清出积累的imu数据，避免积累到下一帧
      uint64_t depth_timestamp =
          static_cast<uint64_t>(depth_image.get_device_timestamp().count());
  ...
  TimeVal depth_image_time_offset_mean =
      TimeVal::Mean(depth_image_time_offsets);
  TimeVal color_image_time_offset_mean =
      TimeVal::Mean(color_image_time_offsets);
  TimeVal depth_image_time_offset_std =
      TimeVal::Std(depth_image_time_offsets, depth_image_time_offset_mean);
  TimeVal color_image_time_offset_std =
      TimeVal::Std(color_image_time_offsets, color_image_time_offset_mean);

  std::cout << "Start to record depth, color, imu data into: " << save_dir
            << std::endl;
  ...
  if ((!save_dir.empty())) {
    ...
    OpenFileOrExit(color_timestamp_file, save_dir + "/rgb.txt",
                   std::ios_base::app);
    color_timestamp_file << "# color images" << std::endl;
    color_timestamp_file << "# timestamp filename" << std::endl;
    color_timestamp_file << "# color_sys_time = color_device_time + mean of "
                            "color_image_time_offset"
                         << std::endl;
    color_timestamp_file << "# time offset for color image, mean: "
                         << color_image_time_offset_mean;
    color_timestamp_file << " std: " << color_image_time_offset_std
                         << std::endl;
    // depth image timestamp
    OpenFileOrExit(depth_timestamp_file, save_dir + "/depth.txt",
                   std::ios_base::app);
    depth_timestamp_file << "# depth images" << std::endl;
    depth_timestamp_file << "# timestamp filename" << std::endl;
    depth_timestamp_file << "# depth_sys_time = depth_device_time + mean of "
                            "depth_image_time_offset"
                         << std::endl;
    depth_timestamp_file << "# time offset for depth image, mean: "
                         << depth_image_time_offset_mean;
    depth_timestamp_file << " std: " << depth_image_time_offset_std
                         << std::endl;
    // imu data timestamp
    OpenFileOrExit(imu_file, save_dir + "/imu.txt", std::ios_base::app);
    imu_file << "# imu timestamp is aligned with depth image " << std::endl;
    imu_file << "# which means imu_sys_time = imu_device_time + mean of "
                "depth_image_time_offset"
             << std::endl;
    ...
    OpenFileOrExit(color_intrinsic_file, save_dir + "/color_intrinsic.txt",
                   std::ios_base::out);
    color_intrinsic_file << new_camera_matrix << std::endl;
    color_intrinsic_file.close();
    std::ofstream depth_intrinsic_file;
    OpenFileOrExit(depth_intrinsic_file, save_dir + "/depth_intrinsic.txt",
                   std::ios_base::out);
...
#ifdef DEBUG_STD_COUT
  std::cout << "time offset for depth image, mean: "
            << depth_image_time_offset_mean;
  std::cout << " std: " << depth_image_time_offset_std << std::endl;
  std::cout << "time offset for color image, mean: "
            << color_image_time_offset_mean;
  ...
  // viewer->setCameraPosition(-493.926, -2538.05, -4271.43, 0.0244369,
  // -0.907735, 0.418832, 0);
  viewer->registerKeyboardCallback(&keyboardEventOccurred, (void *)NULL);

  enum { kStop = 0, kPause = 1, kRecord = 2 } control_flag;
  for (control_flag = kRecord; control_flag != kStop;) {
    ...
    if (device.get_capture(&capture)) {
      TimeVal sys_time = TimeVal::GetCurrentSysTime();
      // rgb
      // * Each pixel of BGRA32 data is four bytes. The first three bytes
      // represent Blue, Green,
      // * and Red data. The fourth byte is the alpha channel and is unused in
      // the Azure Kinect APIs.
      color_image = capture.get_color_image();
      cv_rgba_image =
          cv::Mat(src_color_size, CV_8UC4, (void *)color_image.get_buffer());
      ...
      // depth data. The unit of the data is
      // * in millimeters from the origin of the camera.
      depth_image = capture.get_depth_image();
      // cv_depth_image = cv::Mat(depth_image.get_height_pixels(),
      // depth_image.get_width_pixels(), CV_16UC1, 					 (void
      //*)depth_image.get_buffer());//,
      //static_cast<size_t>(depth_image.get_stride_bytes()));

      transformed_depth_image =
          transformation.depth_image_to_color_camera(depth_image);
      cv_transformed_depth_image = cv::Mat(
          src_color_size, CV_16U, (void *)transformed_depth_image.get_buffer(),
          static_cast<size_t>(transformed_depth_image.get_stride_bytes()));
      // DrawDepth(cv_transformed_depth_image);

      point_cloud_image = transformation.depth_image_to_point_cloud(
          transformed_depth_image, K4A_CALIBRATION_TYPE_COLOR);

      int16_t *point_cloud_image_data =
          (int16_t *)(void *)point_cloud_image.get_buffer();
      const uint8_t *color_image_data = color_image.get_buffer();

      point_cloud.clear();
      for (auto i = 0; i < src_color_size.area(); i++) {
        Vertex vertex;
        vertex.x = point_cloud_image_data[3 * i + 0] / 1.0f;
        vertex.y = point_cloud_image_data[3 * i + 1] / 1.0f;
        vertex.z = point_cloud_image_data[3 * i + 2] / 1.0f;
        if (IsValidVertex(vertex)) {
          pcl::PointXYZRGB point;
          point.x = -1.0 * vertex.x;
          point.y = -1.0 * vertex.y;
          point.z = vertex.z;
      ...
      cv::resize(cv_rgb_image, cv_scaled_rgb_image, tgt_color_size, 0, 0,
                 cv::INTER_AREA);
      cv::resize(cv_transformed_depth_image, cv_scaled_depth_image,
                 tgt_color_size, 0, 0, cv::INTER_NEAREST);
      // DrawDepth(cv_scaled_depth_image);

      cv::remap(cv_scaled_rgb_image, cv_undistorted_rgb_image, map1, map2,
                cv::INTER_LINEAR, cv::BORDER_CONSTANT);
      cv::remap(cv_scaled_depth_image, cv_undistorted_depth_image, map1, map2,
      ...
      if (save_colorized_depth) {
        ...
        std::pair<uint16_t, uint16_t> expectedValueRange = {
            min_depth_for_colorize, max_depth_for_colorize};
        ColorizeDepthImage(cv_undistorted_depth_image,
                           DepthPixelColorizer::ColorizeBlueToRed,
                           expectedValueRange, &pixel_buffer);
        cv_colorized_depth_image =
            cv::Mat(tgt_color_size, CV_8UC4, pixel_buffer.data());
      }
      ...
      while (device.get_imu_sample(&imu_sample, std::chrono::milliseconds(0))) {
        ...
        if (control_flag == kRecord && !save_dir.empty()) {
          ...
          TimeVal imu_systime =
              TimeVal(std::chrono::microseconds(imu_timestamp)) +
              depth_image_time_offset_mean;
...
#endif
      if (control_flag == kRecord && !save_dir.empty()) {
        TimeVal depth_systime = (TimeVal(depth_image.get_device_timestamp()) +
                                 depth_image_time_offset_mean)
                                    .Validate();
        std::string depth_fn =
            "depth/" + depth_systime.GetTimeStampStr() + ".png";
        cv::imwrite(save_dir + "/" + depth_fn, cv_undistorted_depth_image);
        if (save_colorized_depth) {
          std::string colorized_depth_fn =
              "colored_depth/" + depth_systime.GetTimeStampStr() + ".png";
          cv::imwrite(save_dir + "/" + colorized_depth_fn,
                      cv_colorized_depth_image);
        }
        TimeVal color_systime = (TimeVal(color_image.get_device_timestamp()) +
                                 color_image_time_offset_mean)
                                    .Validate();
        std::string color_fn =
            "color/" + color_systime.GetTimeStampStr() + ".png";
        cv::imwrite(save_dir + "/" + color_fn, cv_undistorted_rgb_image);

        color_timestamp_file << color_systime.GetTimeStampStr() << " "
                             << color_fn << std::endl;
        depth_timestamp_file << depth_systime.GetTimeStampStr() << " "
                             << depth_fn << std::endl;
      }
    } else {
