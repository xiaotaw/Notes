...
int main(int argc, char **argv)
{
    ...
    std::string vert_shader_filename = project_dir + "/visualize/shaders/pointcloud.vert";
    std::string frag_shader_filename = project_dir + "/visualize/shaders/pointcloud.frag";
    GLRender render(vert_shader_filename, frag_shader_filename);

    for (unsigned i = 0; (!glfwWindowShouldClose(render.window_)) && (i < color_fn_list.size());)
    {
        if (update)
        {
