...
namespace device{
    ...
    template<typename T>
    __global__ void ResizeDownNearestKernel(cudaTextureObject_t src_texture, 
...
template<typename T>
void CudaTextureSurface2D<T>::ResizeDownNearest(CudaTextureSurface2D<T> &dst, cudaStream_t stream){
    ...
    device::ResizeDownNearestKernel<T><<<grid, blk, 0, stream>>>(
        texture_,
        cols_, rows_,
        dst.cols_, dst.rows_,
...
template<typename T>
void CudaTextureSurface2D<T>::ResizeDownInterArea(CudaTextureSurface2D<T> &dst, cudaStream_t stream){
    // float col_factor_inv = static_cast<float>(cols_ / dst.cols_);
    // float row_factor_inv = static_cast<float>(rows_ / dst.rows_);
    // dim3 blk(16, 16);
    // dim3 grid(DivideUp(dst.cols_, blk.x), DivideUp(dst.rows_, blk.y));
...
template<typename T>
void CudaTextureSurface2D<T>::ResizeDown(CudaTextureSurface2D<T> &dst, cudaStream_t stream, InterpolationMode mode){
    if (mode == kNearest){
...
template<>
void CudaTextureSurface2D<ushort>::ResizeDown(CudaTextureSurface2D<ushort> &dst, cudaStream_t stream, InterpolationMode mode){
    if (mode == kNearest){
...
template<>
void CudaTextureSurface2D<uchar4>::ResizeDown(CudaTextureSurface2D<uchar4> &dst, cudaStream_t stream, InterpolationMode mode){
    if (mode == kNearest){
