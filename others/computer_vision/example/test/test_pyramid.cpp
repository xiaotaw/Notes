...
static void DrawDepthImage(const std::string &win_name, const cv::Mat &depth_img)
{
...
int main(int argc, char **argv)
{
    ...
    for (unsigned i = 0; i < color_fn_list.size(); i++)
    {
        ...
        cv::Mat color_img_ = cv::imread(data_dir + color_fn_list[i], CV_ANYCOLOR | CV_ANYDEPTH);
        cv::Mat color_img;
